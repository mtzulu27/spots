...
import { getContextualSpotCategory, type ExploreSort } from '@/lib/explore-filters';
...
export type ExploreDiscoveryProps = {
  sortBy?: ExploreSort;
  getLikesCount?: (id: string | number) => number;
  resultsMode?: boolean;
  listing?: boolean;
  avatar: string | null;
  name: string;
  ...
  onSearchBack?: () => void;
  onFilters: (inMap?: boolean) => void;
  onMapCategory?: (value: string) => void;
...
export function ExploreDiscovery(p: ExploreDiscoveryProps) {
  ...
  const stickySearchPaddingTop = Platform.OS === 'web' && isStandalonePwa && stickySearchActive
    ? insets.top + 32
    : 16;
  const stickySearchMotionStyle: any = Platform.OS === 'web'
    ? { transitionProperty: 'padding-top', transitionDuration: '220ms', transitionTimingFunction: 'ease-out' }
    : undefined;
  const safeTopStyle = { paddingTop: 0 };
  const returnScroll = useReturnScroll('discovery');
  const searchReturnScroll = useReturnScroll('search');
  const scroll = returnScroll.ref;
  ...
  const [feedEntrance, setFeedEntrance] = useState(0);
  useEffect(() => {
    const subscription = AppState.addEventListener('change', state => {
      if (state === 'active') setFeedEntrance(value => value + 1);
    });
    return () => subscription.remove();
  }, []);
  useFocusEffect(useCallback(() => {
    setFeedEntrance(value => value + 1);
    setStickySearchActive(false);
    setStickySearchTop(null);
  }, []));
  ...
  const { today: nearby } = useMemo(() => {
    if (searching || mapOpen) return { featured: [], today: [] };
  ...
  }, [p.spots, userLocation, zone, now, preferences.radius, searching, mapOpen]);
  ...
  const sortedSearchResults = useMemo(() => searching ? rankSearchResults(searchCatalog, deferredSearchQuery, now, p.sortBy, p.getLikesCount) : [], [searching, searchCatalog, deferredSearchQuery, now, p.sortBy, p.getLikesCount]);
  // Explore's default feed is intentionally presented in reverse alphabetical
  // order while places are being added to the catalog from A to Z.
  const today = [...nearby].sort((a, b) => b.spot.name.localeCompare(a.spot.name, 'es', { sensitivity: 'base' }));
  const nearbyOpenNow = useMemo(() => nearby
    .filter(item => item.availability === 0 && (!userLocation || item.distance !== null))
    .sort((a, b) => userLocation
      ? (a.distance ?? Infinity) - (b.distance ?? Infinity)
      : 0)
    .slice(0, 10)
    .map(item => item.spot), [nearby, userLocation]);
  ...
  const setSelectedZone = (value: string) => {
    pendingScrollRestoreY.current = currentScrollY.current;
    setZone(value);
  };
  useEffect(() => {
    ...
    requestAnimationFrame(() => scroll.current?.scrollTo({ y: restoreY, animated: false }));
  }, [zone]);
  const renderZoneChips = () => (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.railContent}>
      {['', ...zones].map(value => <Pressable key={value} accessibilityRole="button" accessibilityState={{ selected: zone === value }} onPress={() => setSelectedZone(value)} style={[s.zone, zone === value && s.circleActive]}><Text style={s.meta}>{value || 'Todas las zonas'}</Text></Pressable>)}
    </ScrollView>
  );
  function renderCards(items: typeof today, columnCount: number, width: number) {
return (width > 0 && items.map(({ spot, distance, label }, index) => <Entrance key={spot.id} index={index} trigger={`${resultsKey}:${zone}:${columnCount}`} style={{ width: (width - 16 * (columnCount - 1)) / columnCount }}><ExploreGridCard spot={spot} displayCategory={getContextualSpotCategory(spot, p.selected, deferredSearchQuery)} showLogo columnCount={columnCount} distance={distance} label={label} bookmarked={p.isSaved(spot.likeTargetId)} onPress={() => router.push(`/spot/${spot.id}`)} onToggleBookmark={() => p.toggleSaved(spot.likeTargetId)} /></Entrance>));
  }
  function renderDiscoveryCards(items: Spot[]) {
    return items.map((spot, index) => <Entrance key={spot.id} index={index} trigger={resultsKey}>
      <DiscoveryPlaceCard spot={spot} displayCategory={getContextualSpotCategory(spot, p.selected, deferredSearchQuery)} now={now} bookmarked={p.isSaved(spot.likeTargetId)} onPress={() => open(spot)} onToggleBookmark={() => p.toggleSaved(spot.likeTargetId)} />
    </Entrance>);
  }
  ...
  if (searching) {
    ...
    return (
      <Entrance style={s.screen} trigger="search">
        <View style={[s.searchHeader, { paddingTop: topContentInset(insets) }]}>
          <AppIconButton name="arrow-back" accessibilityLabel="Volver a explorar" tone="light" size={42} onPress={() => { returnedFromSearch.current = true; setSearching(false); setZone(''); setSearchLimit(20); p.onQuery(''); p.onSearchBack?.(); }} />
        ...
        <ScrollView {...searchReturnScroll} keyboardShouldPersistTaps="handled" contentContainerStyle={[s.searchResults, { paddingTop: hasResults ? 8 : 16, paddingBottom: insets.bottom + 24 }]}>
          <View style={s.searchTools}>
            ...
            <View style={s.searchActions}>
              ...
              <Pressable accessibilityRole="button" accessibilityLabel={p.activeFiltersCount ? `Abrir filtros, ${p.activeFiltersCount} activos` : 'Abrir filtros'} onPress={() => p.onFilters(false)} style={s.filter}>
  ...
  return (
    <View style={[s.screen, safeTopStyle]}>
      <Entrance style={s.screen} trigger="home" disabled={returnedFromSearch.current}>
      <ScrollView {...returnScroll} stickyHeaderIndices={Platform.OS === 'web' || p.listing ? undefined : [1]} onScroll={event => { returnScroll.onScroll(event); const offsetY = event.nativeEvent.contentOffset.y; currentScrollY.current = offsetY; if (Platform.OS === 'web' && stickySearchTop != null) setStickySearchActive(offsetY >= stickySearchTop - 1); }} scrollEventThrottle={16} showsVerticalScrollIndicator={false} contentContainerStyle={[s.page, { paddingTop: topContentInset(insets), paddingBottom: (isStandalonePwa ? 156 : 136) + insets.bottom }]}> 
        ...
        {!p.listing && <View onLayout={event => { const top = event.nativeEvent.layout.y; setStickySearchTop(top); if (Platform.OS === 'web') setStickySearchActive(currentScrollY.current >= top - 1); }} style={[s.stickySearchShell, Platform.OS === 'web' && { position: 'sticky' as any, top: 0 }]}> 
        <View style={[s.stickySearch, { paddingTop: stickySearchPaddingTop }, stickySearchMotionStyle]}>
        <View style={s.search}>
        ...
        <View style={s.section}>
          <View style={s.sectionHead}><Text style={s.heading}>Cerca y abierto ahora</Text><Pressable onPress={p.onAll} accessibilityRole="button"><Text style={s.more}>Ver todos</Text></Pressable></View>
          <Entrance trigger={`nearby-${feedEntrance}`}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.rail} contentContainerStyle={s.railContent}>
            {renderDiscoveryCards(nearbyOpenNow)}
          </ScrollView>
          </Entrance>
          {!nearbyOpenNow.length && <Text style={s.empty}>No encontramos lugares abiertos cerca en este momento.</Text>}
        </View>
        </>}
        <View style={s.section}>
          <View style={s.sectionHead}>
            <Text style={s.heading}>{p.listing ? 'Cerca y abierto' : 'Para visitar hoy'}</Text>
            {!p.listing && <GridColumnSelector columns={columns} onChange={setColumns} />}
          </View>
          <View
            style={[
              s.zoneStickyRow,
              Platform.OS === 'web' && { position: 'sticky' as any, top: insets.top + 72 },
            ]}
          >{renderZoneChips()}</View>
          <Entrance trigger={`today-${feedEntrance}`}>
          <View style={s.grid} onLayout={event => setGridWidth(event.nativeEvent.layout.width)}>
            {renderCards(today, columns, gridWidth)}
          </View>
          </Entrance>
          {!today.length && <Text style={s.empty}>No hay lugares en esta zona con los filtros actuales.</Text>}
        </View>
      </ScrollView>
      {!p.listing && <DiscoveryNavigation active="explore" onExplore={() => scroll.current?.scrollTo({ y: 0, animated: true })} onMap={() => setMapOpen(true)} onParches={() => router.replace('/today')} onContribute={() => router.replace('/contribute')} onAccount={() => router.replace('/account')} />}
      </Entrance>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  stickySearchShell: { height: 72, zIndex: 10 },
  profileText: { flex: 1, gap: 4 },
  greeting: { fontSize: 18, fontWeight: '600', color: ui.text },
  inline: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  meta: { fontSize: 12, lineHeight: 17, color: ui.textSecondary },
  search: { flexDirection: 'row', alignItems: 'center', borderRadius: 999, borderWidth: 1, borderColor: ui.border, backgroundColor: ui.surface, paddingRight: 4 },
  ...
  stickySearch: { backgroundColor: ui.bg, marginHorizontal: -20, marginTop: -16, paddingHorizontal: 20, paddingBottom: 8, marginBottom: -12, zIndex: 10 },
  zoneStickyRow: { zIndex: 9, backgroundColor: ui.bg, marginHorizontal: -20, paddingHorizontal: 20, paddingVertical: 8, marginVertical: -8 },
