...
async function uploadImage(supabase, spotSlug, kind, filePath, index) {
  ...
  const assetPath = `${slugify(spotSlug)}/${kind}/lumina-${kind}-${index}-${Date.now()}.${extension}`;
...
async function main() {
  ...
  const spotSlug = 'lumina';
  const coverPath =
    '/Users/mateo/Downloads/Instagram/imgi_3_660610639_1659947505433137_1570431051228087386_n.jpg';
  const galleryPaths = [
    '/Users/mateo/Downloads/Instagram/imgi_3_660610639_1659947505433137_1570431051228087386_n.jpg',
    '/Users/mateo/Downloads/Instagram/imgi_11_645917573_979761011889402_6279810775604481590_n.jpg',
  ];
  ...
  if (entry) {
    entry.internalTag = 'followup-address-hours-pricing-contact';
    entry.missingFields = entry.missingFields.filter((field) => field !== 'photos');
    entry.notes = [
      ...entry.notes.filter((note) => !note.toLowerCase().includes('fotos')),
      'Se cargó una primera tanda manual de fotos útiles: coctelería/ambiente y frente nocturno del lugar.',
    ];
